...
def get_plane_project_members() -> List[Dict]:
    """
    Fetch project members from Plane
    """
    try:
...
def map_zalo_user_to_plane_format(plane_member: Dict, zalo_user: Optional[Dict]) -> Dict:
    ...
    mapped_user = {
        ...
        "member_id": plane_member.get("member_id"),
...
def get_enriched_project_members() -> List[Dict]:
    """
    Get Plane project members enriched with Zalo user data
    """
    plane_members = get_plane_project_members()
    enriched_members = []
...
if __name__ == "__main__":
    enriched_members = get_enriched_project_members()
